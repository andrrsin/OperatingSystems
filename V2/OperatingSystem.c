...
void OperatingSystem_HandleClockInterrupt()
{
	...
	if (temp > numberOfSleepingProcesses)
	{
		OperatingSystem_PrintStatus();
		
		if (Heap_getFirst(readyToRunQueue[USERPROCESSQUEUE], numberOfReadyToRunProcesses[USERPROCESSQUEUE])>0 && (processTable[executingProcessID].queueID == DAEMONSQUEUE ||
		...
		else if (Heap_getFirst(readyToRunQueue[DAEMONSQUEUE], numberOfReadyToRunProcesses[DAEMONSQUEUE])>0 && 
